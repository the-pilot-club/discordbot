    ...
    try {
        ...
        switch (response.status) {
            ...
            case 201:
                ...
                break
            case 208:
                // user was already found, so did not add, no logging needed
                break;
            case 401:
                // Token is not working properly, logging to sentry
                sendToSentry(`Received 401 when adding user ${member.id} - Unauthorized.`, "Adding user to FCP via Discord");
                break;
            case 422:
                // User somehow not in discord but is being added via API
